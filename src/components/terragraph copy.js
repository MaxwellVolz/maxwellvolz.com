...
export default function Terragraph(props) {
  ...
  useFrame(({ clock }) => {
    ...
    if (meshRef1.current.position.x > -5.75) {
      // console.log("x", meshRef1.current.position.x)
      meshRef1.current.position.x -= 0.02;
    }
    else {
      console.log("blink")
      meshRef1.current.position.x = 0;
    }


    if (meshRef2.current.position.x < 5.75) {
      meshRef2.current.position.x += 0.02;
    }
    else {
      console.log("blink")
      meshRef2.current.position.x = 0;
    }

    if (meshRef3.current.position.z < 7.4) {
      meshRef3.current.position.z += 0.02;
      meshRef3.current.position.x += 0.0006;
    }
    else {
      console.log("blink")
      meshRef3.current.position.z = 0;
      meshRef3.current.position.x = 0;
